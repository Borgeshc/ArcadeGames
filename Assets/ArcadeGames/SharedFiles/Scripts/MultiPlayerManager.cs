...
public class MultiPlayerManager : MonoBehaviour
{
    ...
    PlayerInput FindPlayerUsingDevice(InputDevice inputDevice)
    {
        var playerCount = players.Count;
        for (var i = 0; i < playerCount; i++)
        {
            var player = players[i];
            if (player.inputDevices == inputDevice)
            {
                return player;
            }
        }
    ...
    PlayerInput CreatePlayer(InputDevice inputDevice)
    {
        if (players.Count < maxPlayers)
        {
            ...
            player.inputDevices = inputDevice;
            players.Add(player);
